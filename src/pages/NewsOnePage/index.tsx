...
function NewsOnePage({ }: Props) {
  ...
  useEffect(() => {
    axios.get(`${apiUrl}/news/${params.id}`).then((resp) => {
        ...
        <div className='grid grid-cols-12 gap-5'>
          ...
          <div className='flex flex-col sm:items-end col-span-12 sm:col-span-5 gap-[50px]'>
            ...
            <div className='sm:max-w-[400px] grid gap-[50px]'>
              {news1 && <NewsCard id={news1.id} title={news1.title} description={news1.description} createdDate={news1.createdDate} img={news1.imageUrl} />}
              {news2 && <NewsCard id={news2.id} title={news2.title} description={news2.description} createdDate={news2.createdDate} img={news2.imageUrl} />}
            </div>
          </div>
        </div>
      </div>
